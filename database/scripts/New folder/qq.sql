  ...
  SELECT 
    ...
    CAST(EXTRACT(YEAR FROM a.DATA_DESFILIACAO) AS UNSIGNED) AS ANO_DF,
    CAST(EXTRACT(MONTH FROM a.DATA_DESFILIACAO) AS UNSIGNED) AS MES_DF
FROM
    (SELECT DISTINCT
        cp.id AS id,
...
ORDER BY a.id , CAST(EXTRACT(YEAR FROM a.DATA_FILIACAO) AS UNSIGNED) , CAST(EXTRACT(MONTH FROM a.DATA_FILIACAO) AS UNSIGNED)